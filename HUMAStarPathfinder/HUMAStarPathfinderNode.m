...
-(float)distanceFromTarget:(HUMAStarPathfinderNode*)targetCoordinate ofCurrentObject:(HUMAStarPathfinderNode*)objectCoordinate{

    
    int deltaX = abs(objectCoordinate.tileLocation.x - targetCoordinate.tileLocation.x);
    int deltaY = abs(objectCoordinate.tileLocation.y - targetCoordinate.tileLocation.y);
...
- (NSArray *)findPathFromStart:(CGPoint)start toTarget:(CGPoint)target {
	...
	// The algorithm calculates the node closer to the target
        ...
        int index=0;
        int i=0;
        for (HUMAStarPathfinderNode*currentNode in neighbors){
            float currentDistance=[self distanceFromTarget:self.targetNode ofCurrentObject:currentNode];
        ...
        self.targetNode=[neighbors objectAtIndex:index];
    ...
    NSLog(@"%d",self.shortestPath.count);
